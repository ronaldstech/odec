        ...
        <div className="bg-white">
            ...
            <section className="py-32 relative bg-slate-50/30 overflow-hidden">
                ...
                <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
                    <div className="absolute -top-[20%] -right-[10%] w-[40rem] h-[40rem] rounded-full bg-gradient-to-br from-[var(--color-primary-100)]/40 to-transparent blur-3xl" />
                    <div className="absolute -bottom-[20%] -left-[10%] w-[40rem] h-[40rem] rounded-full bg-gradient-to-tr from-emerald-100/40 to-transparent blur-3xl" />
                </div>
                ...
                <div className="container mx-auto px-6 relative z-10">
                    <div className="text-center max-w-3xl mx-auto mb-20">
                        <motion.div
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-50 border border-emerald-100/50 text-emerald-600 font-bold tracking-widest uppercase text-xs mb-6"
                        >
                            <Users2 className="w-4 h-4" />
                            The People
                        </motion.div>
                        <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 mb-6 tracking-tight">Meet the <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-[var(--color-primary-600)]">Change Makers</span></h2>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-10">
                        {[
                            ...
                            },
                            {
                                ...
                                name: "RABECCA MULUNGU",
                                image: "rabecca.jpeg",
                                icon: <Globe className="w-5 h-5" />,
                                bio: "Experienced professional with a robust background in renewable energy, administration, and Agri-business. Dedicated to integrating sustainable agricultural practices and innovative energy solutions to drive community resilience and environmental conservation.",
                                contacts: { email: "#", phone: "#", linkedin: "#", twitter: "#" }
                            }
                        ].map((member, i) => (
                            <motion.div
                                ...
                                viewport={{ once: true }}
                                className="group relative bg-white/80 backdrop-blur-xl rounded-[2.5rem] p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] transition-all duration-500 border border-white hover:border-[var(--color-primary-100)] flex flex-col h-full"
                            >
                                {/* Glow effect behind card on hover */}
                                <div className="absolute inset-0 bg-gradient-to-b from-[var(--color-primary-500)]/0 to-[var(--color-primary-500)]/5 rounded-[2.5rem] opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />

                                <div className="relative aspect-[4/5] rounded-[2rem] overflow-hidden bg-slate-100 mb-6 border border-black/5 shadow-inner">
                                    ...
                                    <div className="absolute bottom-4 left-4 right-4">
                                        <div className="transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                                            <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white text-xs font-bold uppercase tracking-widest shadow-xl">
                                                {member.role}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="px-2 flex-grow flex flex-col relative z-10">
                                    ...
                                    {/* Default role view (hides on hover) */}
                                    <p className="text-[var(--color-primary-500)] text-xs font-bold uppercase tracking-widest mb-4 group-hover:opacity-0 group-hover:h-0 transition-all duration-300 overflow-hidden">
                                        {member.role}
                                    </p>

                                    {member.bio && (
                                        <p className="text-slate-500 text-sm leading-relaxed font-medium mb-6 line-clamp-3 group-hover:line-clamp-none transition-all duration-500">
                                            {member.bio}
                                        </p>
                                    )}
                                        ...
                                        {member.contacts && (
                                            <div className="flex gap-2.5">
                                                {member.contacts.email && (
                                                    <a href={member.contacts.email !== "#" ? `mailto:${member.contacts.email}` : "#"} className="w-9 h-9 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center border border-slate-100 shadow-sm hover:bg-[var(--color-primary-50)] hover:text-[var(--color-primary-600)] hover:border-[var(--color-primary-200)] hover:-translate-y-1 transition-all duration-300" title="Email">
                                                        <Mail className="w-4 h-4" />
                                                    </a>
                                                )}
                                                {member.contacts.phone && (
                                                    <a href={member.contacts.phone !== "#" ? `tel:${member.contacts.phone}` : "#"} className="w-9 h-9 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center border border-slate-100 shadow-sm hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 hover:-translate-y-1 transition-all duration-300" title="Phone">
                                                        <Phone className="w-4 h-4" />
                                                    </a>
                                                )}
                                                {member.contacts.linkedin && (
                                                    <a href={member.contacts.linkedin} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center border border-slate-100 shadow-sm hover:bg-[#0077b5]/10 hover:text-[#0077b5] hover:border-[#0077b5]/30 hover:-translate-y-1 transition-all duration-300" title="LinkedIn">
                                                        <Linkedin className="w-4 h-4" />
                                                    </a>
                                                )}
                                                {member.contacts.twitter && (
                                                    <a href={member.contacts.twitter} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center border border-slate-100 shadow-sm hover:bg-[#1DA1F2]/10 hover:text-[#1DA1F2] hover:border-[#1DA1F2]/30 hover:-translate-y-1 transition-all duration-300" title="Twitter">
